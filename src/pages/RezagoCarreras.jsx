...
export default function RezagoCarreras() {
  ...
  useEffect(() => {
    obtenerCarreras().then(setCarreras).catch(() => setCarreras([]));
  }, []);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        // Nota: tu endpoint devuelve un historial por materia.
        // Para el formato imprimible agrupamos por alumno+materia (sin duplicar en exceso).
  ...
  const carrera = useMemo(
    () => carreras.find((c) => c.id_carrera === carreraActiva || c.id === carreraActiva),
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-4xl font-bold text-slate-900">Rezago por carrera</h1>
            <p className="mt-1 text-slate-500">Selecciona una carrera para ver los alumnos con materias reprobadas.</p>
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  className={`w-full rounded-xl border px-3 py-2.5 text-left transition ${
                    activa ? "border-green-300 bg-green-50" : "border-transparent hover:bg-slate-50"
                  }`}
                >
                  <p className={`text-sm font-semibold ${activa ? "text-green-800" : "text-slate-800"}`}>{nombre}</p>
                  <p className="mt-0.5 text-xs text-slate-400">{activa ? lista.length : ""}</p>
                </button>
              );
            })}
          </div>
        ...
        <section className="overflow-auto rounded-2xl border border-slate-200 bg-slate-100 p-4 shadow-sm">
          {!carreraActiva ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 py-20 text-slate-400">
              ...
              <p className="text-sm">Selecciona una carrera para ver el formato</p>
            </div>
          ) : (
            <article id="rezago-preview" className="rezago-hoja rezago-doc mx-auto shadow-sm">
              <p className="rezago-inst">CENTRO DE ESTUDIOS SUPERIORES DE LA FRONTERA, UNIFRONT</p>
              <p className="rezago-dept">DEPARTAMENTO DE CONTROL ESCOLAR</p>

              <h1 className="rezago-carrera-titulo">LICENCIATURA EN {carrera?.nombre || ""}</h1>
              ...
              <footer className="rezago-footer">
                BOULEVARD BERNARDO O' HIGGINS #6050 3RA. ETAPA ZONA RIO&nbsp;&nbsp; TEL. + 52 (664) 660.1989
              </footer>
            </article>
          )}
        </section>
